...
There are few configurations settings to control the output format.
 ...
 - label_keys: (default: "job,instance") comma separated list of keys to use as stream labels. If the value of the key is a Hash, then all key-value pair in the hash will be used as stream labels. All other keys will be placed into the log line
